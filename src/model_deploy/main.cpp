...
void AngleMsr_End(Arguments *in, Reply *out);
...
RPCFunction rpcAngle(&AngleMsr, "AngleMsr");
RPCFunction rpcAngle_End(&AngleMsr_End, "AngleMsr_End");
...
int mode1 = 0;
int mode2 = 0;
const char* topic1 = "Gesture";
const char* topic2 = "Angle";
...
void Gesture_main(void)
{
  ...
  while (mode1 == 0) {
    ...
    while ((state == 0) && (mode1 == 0)) {
      gestureDctQueue.call(&Gesture_detect);
      ThisThread::sleep_for(100ms);
    }
    if (mode1 == 0) {
      ulcd_draw(sel, 1);
      if (sel == 0) {
        sprintf(buffer, "Threshold angle = 30");
      } else if (sel == 1) {
        sprintf(buffer, "Threshold angle = 45");
      } else if (sel == 2) {
        sprintf(buffer, "Threshold angle = 60");
      } else {
        sprintf(buffer, "ERROR sel out of range");
      }
      printf("%s\n", buffer);
      mqtt_queue.call(&publish_message, client_global, buffer, 0);
    }
